...
namespace MusicGame.Device
{
    ...
    class Renderer
    {
        ...
        public void Begin(SpriteSortMode Deferred,
        ...
        public void DrawTexture(string assetName, Vector2 position, Rectangle? rect, Color color, float rotation, Vector2 origin, float scale, SpriteEffects effects, float alpha = 1.0f)
        {
            //デバッグモードの時のみ、画像描画前のアセット名チェック
            Debug.Assert(
                textures.ContainsKey(assetName),
                "描画時にアセット名の指定を間違えたか、" +
                "画像の読み込み自体できていません");
            origin = new Vector2(textures[assetName].Width / 2, textures[assetName].Height / 2);
